...
namespace bb.Core.Commands.List.Games
{
    public class Cookie : CommandBase
    {
        ...
        public override async Task<CommandReturn> ExecuteAsync(CommandData data)
        {
            ...
            try
            {
                if (data.ChannelId == null || Program.BotInstance.UsersBuffer == null || Program.BotInstance.DataBase == null)
                {
                ...
                if (data.Arguments is not null && data.Arguments.Count > 0)
                {
                    if (statsAliases.Contains(data.Arguments[0].ToLower()))
                    {
                        ...
                        long eaten = Convert.ToInt64(Program.BotInstance.UsersBuffer.GetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.EatedCookies));
                        long gifted = Convert.ToInt64(Program.BotInstance.UsersBuffer.GetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.GiftedCookies));
                        long received = Convert.ToInt64(Program.BotInstance.UsersBuffer.GetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.ReceivedCookies));
                    ...
                    else if (giftAliases.Contains(data.Arguments[0].ToLower()))
                    {
                        ...
                        {
                            commandReturn.SetMessage(LocalizationService.GetString(data.User.Language, "error:not_enough_arguments", data.ChannelId, data.Platform, $"{Program.BotInstance.DefaultCommandPrefix}cookie gift username"));
                            return commandReturn;
                        }

                        DateTime gifterLastUse = DateTime.Parse((string)Program.BotInstance.UsersBuffer.GetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.LastCookie), null, DateTimeStyles.AdjustToUniversal);
                        ...
                        DateTime recipientLastUse = DateTime.Parse((string)Program.BotInstance.UsersBuffer.GetParameter(data.Platform, DataConversion.ToLong(targetUserId), Users.LastCookie), null, DateTimeStyles.AdjustToUniversal);
                        ...
                        Program.BotInstance.UsersBuffer.SetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.GiftedCookies, (int)Program.BotInstance.UsersBuffer.GetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.GiftedCookies) + 1);
                        Program.BotInstance.UsersBuffer.SetParameter(data.Platform, DataConversion.ToLong(targetUserId), Users.ReceivedCookies, (int)Program.BotInstance.UsersBuffer.GetParameter(data.Platform, DataConversion.ToLong(targetUserId), Users.ReceivedCookies) + 1);
                        Program.BotInstance.UsersBuffer.SetParameter(data.Platform, DataConversion.ToLong(targetUserId), Users.LastCookie, DateTime.UtcNow.AddDays(-1).ToString("o"));
                    ...
                    else if (topAliases.Contains(data.Arguments[0].ToLower()))
                    {
                        ...
                        var leaderboard = Program.BotInstance.DataBase.Games.GetLeaderboard("Cookies", data.Platform, $"{topType}Count", 0);
                    ...
                    {
                        DateTime lastUse = DateTime.Parse((string)Program.BotInstance.UsersBuffer.GetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.LastCookie), null, DateTimeStyles.AdjustToUniversal);
                        ...
                        decimal currency = Program.BotInstance.InBankDollars / Program.BotInstance.Coins;
                        decimal cost = 0.2m / currency;

                        if (Program.BotInstance.Currency.Get(data.User.Id, data.Platform) >= cost)
                        {
                            Program.BotInstance.Currency.Add(data.User.Id, cost, data.Platform);
                            Program.BotInstance.UsersBuffer.SetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.LastCookie, DateTime.UtcNow.AddDays(-1).ToString("o"));
                ...
                DateTime lastUsed = DateTime.Parse((string)Program.BotInstance.UsersBuffer.GetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.LastCookie), null, DateTimeStyles.AdjustToUniversal);
                ...
                string[] result = await Program.BotInstance.AiService.Request(
...
Generate ONLY 4 sentence variations following these rules. The answer must be in the user's language! In your answer write ONLY text! DO NOT indicate sentence numbers! DO NOT write more than 4 sentences!",
                ...
                Program.BotInstance.UsersBuffer.SetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.EatedCookies, (int)Program.BotInstance.UsersBuffer.GetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.EatedCookies) + 1);
                Program.BotInstance.UsersBuffer.SetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.LastCookie, DateTime.UtcNow.ToString("o"));
