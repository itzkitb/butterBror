...
namespace bb.Core.Commands.List.AI
{
    public class AI : CommandBase
    {
        ...
        public override string Help => "[action:clear|models] | [model:<name>] [history:ignore] <query>";
        ...
        public override async Task<CommandReturn> ExecuteAsync(CommandData data)
        {
            ...
            try
            {
                if (data.Arguments == null || Program.BotInstance.UsersBuffer == null || data.Channel == null || data.ChannelId == null)
                {
                    commandReturn.SetMessage(LocalizationService.GetString(data.User.Language, "error:unknown", string.Empty, data.Platform));
                    return commandReturn;
                }

                if (MessageProcessor.GetArgument(data.Arguments, "action") is null)
                {
                    decimal currency = Program.BotInstance.Coins == 0 ? 1 : Program.BotInstance.InBankDollars / Program.BotInstance.Coins;
                    ...
                    if (Program.BotInstance.Currency.Get(data.User.Id, data.Platform) >= cost)
                    {
                        if (data.Arguments.Count < 1)
                            commandReturn.SetMessage(LocalizationService.GetString(data.User.Language, "error:not_enough_arguments", data.ChannelId, data.Platform, $"{Program.BotInstance.DefaultCommandPrefix}ai model:qwen Hello!"));
                        else
                        {
                            ...
                            if (MessageProcessor.GetArgument(data.Arguments, "repetition_penalty") is not null)
                            {
                                try
                                {
                                    repetitionPenalty = DataConversion.ToDouble(MessageProcessor.GetArgument(data.Arguments, "repetition_penalty"));
                            ...
                            if (Program.BotInstance.AiService.models.ContainsKey(model.ToLower()))
                            {
                                Program.BotInstance.MessageSender.Send(Models.Platform.Platform.Twitch, LocalizationService.GetString(data.User.Language, "command:gpt:generating", data.ChannelId, data.Platform),
                            ...
                            string[] result = await Program.BotInstance.AiService.Request(request, data.Platform, model, data.User.Name, data.User.Id, data.User.Language, repetitionPenalty, useHistory);
                ...
                {
                    string argument = MessageProcessor.GetArgument(data.Arguments, "action").ToLower();
                    if (argument is "clear")
                    {
                        Program.BotInstance.UsersBuffer.SetParameter(data.Platform, DataConversion.ToLong(data.User.Id), Users.AiHistory, "[]");
                    ...
                    {
                        List<string> models = Program.BotInstance.AiService.models.Select(model => model.Key).ToList();
