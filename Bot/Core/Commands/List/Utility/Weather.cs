...
namespace bb.Core.Commands.List.Utility
{
    ...
    public class Weather : CommandBase
    {
        ...
        public override string Source => "Utility/Weather.cs";
        ...
        public override string Help => "[location] | [action] [parameters]";
        ...
        {
            CommandReturn commandReturn = new CommandReturn();
            if (Program.BotInstance.UsersBuffer == null)
            {
            ...
            Program.BotInstance.UsersBuffer.SetParameter(platform, userLongId, Users.Location, firstLocation.Name);
            Program.BotInstance.UsersBuffer.SetParameter(platform, userLongId, Users.Latitude, firstLocation.Latitude.ToString(CultureInfo.InvariantCulture));
            Program.BotInstance.UsersBuffer.SetParameter(platform, userLongId, Users.Longitude, firstLocation.Longitude.ToString(CultureInfo.InvariantCulture));
        ...
        private CommandReturn HandleGetLocationAction(Models.Platform.Platform platform, string userId, Language language, string channelId)
        {
            ...
            var userPlace = (string)Program.BotInstance.UsersBuffer.GetParameter(platform, DataConversion.ToLong(userId), Users.Location);
        ...
        private async Task<CommandReturn> HandleGetWeatherActionAsync(WeatherAction action, Models.Platform.Platform platform, string userId, Language language, string channelId)
        {
            ...
            if (string.IsNullOrWhiteSpace(action.Location))
            {
                ...
                var userPlace = (string)Program.BotInstance.UsersBuffer.GetParameter(platform, userLongId, Users.Location);
                var userLat = (string)Program.BotInstance.UsersBuffer.GetParameter(platform, userLongId, Users.Latitude);
                var userLon = (string)Program.BotInstance.UsersBuffer.GetParameter(platform, userLongId, Users.Longitude);
                ...
                var weatherData = await _weatherService.GetCurrentWeatherAsync(lat.Value.ToString(CultureInfo.InvariantCulture), lon.Value.ToString(CultureInfo.InvariantCulture));
                if (weatherData == null || weatherData.Temperature == -400)
                {
                    return CreateErrorReturn("error:place_not_found", platform, language, channelId);
                }
        ...
        private List<LocationResult>? GetSavedLocations(Models.Platform.Platform platform, string userId)
        {
            ...
            string weatherResultLocationsUnworkedString = (string)Program.BotInstance.UsersBuffer.GetParameter(platform, DataConversion.ToLong(userId), Users.WeatherResultLocations);
        ...
        private void SaveSearchResults(List<LocationResult> locations, Models.Platform.Platform platform, string userId)
        {
            ...
            Program.BotInstance.UsersBuffer.SetParameter(platform, DataConversion.ToLong(userId), Users.WeatherResultLocations, DataConversion.SerializeStringList(jsons));
        }
