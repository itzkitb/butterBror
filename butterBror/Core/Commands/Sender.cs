    ...
    public class Sender
    {
        ...
        public static async void SendCommandReply(TwitchMessageSendData data, bool isReply = true)
        {
            ...
            try
            {
                ...
                else if (message.Length > 500)
                {
                    ...
                    {
                        Thread.Sleep(1000);
                        Chat.TwitchReply(data.Channel, data.ChannelID, part2, data.MessageID, data.Language, data.SafeExecute);
                    });
                }
        ...
        public static async void SendCommandReply(DiscordCommandSendData data)
        {
            ...
            try
            {
                ...
                {
                    if ((data.SafeExecute | data.IsEphemeral) || new NoBanwords().Check(data.Message, data.ServerID, PlatformsEnum.Discord) && new NoBanwords().Check(data.Description, data.ServerID, PlatformsEnum.Discord))
                    {
                        if (data.IsEmbed)
                        {
