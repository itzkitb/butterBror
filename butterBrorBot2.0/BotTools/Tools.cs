    ...
    public partial class BotWorker
    {
        public class Tools
        {
            ...
            public static async void ReturnFromAFK(OnMessageReceivedArgs e)
            {
                ...
                {
                    Tools.ErrorOccured(ex.Message, "afkBack");
                }
                ...
                if (NoBanwords.fullCheck(message, e.ChatMessage.RoomId))
                {
                    ...
                    if (Tools.FilterTextWithoutSpaces(message) == "")
                    {
                        send = "";
                    }
                    else
                    ...
                    await ChangeNicknameColorAsync(ChatColorPresets.YellowGreen);
                    SendMsgReply(e.ChatMessage.Channel, e.ChatMessage.RoomId, text.Replace("%user%", e.ChatMessage.Username) + send + " (" + Tools.FormatTimeSpan(Tools.GetTimeTo(UsersData.UserGetData<DateTime>(e.ChatMessage.UserId, "afkTime"), DateTime.UtcNow, false), lang) + ")", e.ChatMessage.Id, lang, true);
                }
            }
            ...
            public static string FormatTimeSpan(TimeSpan timeSpan, string lang)
            {
                try
                {
                    int days = ToNumber(timeSpan.Days.ToString().Replace("-", ""));
                    int hours = ToNumber(timeSpan.Hours.ToString().Replace("-", ""));
                    int minutes = ToNumber(timeSpan.Minutes.ToString().Replace("-", ""));
                    int seconds = ToNumber(timeSpan.Seconds.ToString().Replace("-", ""));
                    if (days > 0 || days < 0)
                    {
                ...
                {
                    ErrorOccured(ex.Message, "wbot3A");
                    return default;
                }
            }
            ...
            public static void SaveBalance(string userID, int plusBalance, int plusFloatBalance)
            {
                ...
                {
                    ErrorOccured(ex.Message, "wbot0A");
                }
            }
            ...
            public static void ErrorOccured(string message, string sector)
            {
                ...
                LogWorker.LogError($"Ошибка! \"{message}\"", sector);
            ...
            public static void SendMessage(string channel, string message, string channelID, string messageID, string lang, bool isSafeEx = false)
            {
                ...
                LogWorker.LogInfo($"Было отправлено сообщение в канал {channel}: {message}", "SMR");
            ...
            {
                ConsoleServer.SendConsoleMessage("commands", "Отправка сообщения...");
                LogWorker.LogInfo($"Был отправлен ответ на сообщение в канал {channel}: {message}", "SMR");
            ...
            public static string FilterCommand(string input)
            {
                ...
                {
                    BotWorker.Tools.ErrorOccured(ex.Message, "wbot1A");
                    return "none";
                }
            }
            // #WBOT 2A
            public static int ToNumber(string input)
            {
                ...
                {
                    BotWorker.Tools.ErrorOccured(ex.Message, "wbot2A");
                    return 0;
                }
            }
            public static async Task<string[]> Get7tvEmotes(string channel)
            {
                var client = new HttpClient();
                var request = new HttpRequestMessage
                {
                    Method = HttpMethod.Get,
                    RequestUri = new Uri($"https://emotes.adamcy.pl/v1/channel/{channel}/emotes/7tv"),
                    Headers =
        {
            ...
            public static string RemoveDuplicateLetters(string text)
            {
                ...
                {
                    Tools.ErrorOccured(ex.Message, "RDL");
                    return string.Empty;
                }
            }
            public static void executedCommand(CommandData data)
            {
                ...
                LogWorker.LogInfo(info, $"cmd\\{data.Name}");
            ...
            public static async Task<WeatherData> Get_weather(string lat, string lon)
            {
                    ...
                    foreach (var ip in ips)
                    {
                        ...
                        if (Data < 10 && !success)
                        {
                            ...
                            else
                            {
                                ...
                                LogWorker.LogWarning(err, "WEATHER");
                            }
                        }
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    ErrorOccured(ex.Message, "WEATHER");
                    return errData;
                }
            }
            public static async Task<List<Place>> Get_location(string placeName)
            {
                    ...
                    if (FoundData.Count == 0)
                    {
                        ...
                        foreach (var ip in ips)
                        {
                                    ...
                                    Headers =
                            {
                                ...
                                {
                                    var err2 = $"\n ОШИБКА API ПОГОДЫ ({ip}): #{response.StatusCode}, {response.ReasonPhrase}";
                                    LogWorker.LogWarning(err2, "WEATHER");
                                }
                            }
                        }
                    }
                ...
                {
                    LogWorker.LogError(ex.Message, "locationGET");
                    Place err = new()
                    {
                        name = "err",
                        lat = "",
            ...
            {
                var selectedUser = GetUsernameFromText(argsText);
                ...
                if (now < winterStart)
                {
                    return TranslationManager.GetTranslation(lang, $"to{type}", channelID)
                        .Replace("%time%", FormatTimeSpan(GetTimeTo(winter, now), lang))
                ...
                if (!(UsersData.UserGetData<bool>(UserID, "isBotModerator") || UsersData.UserGetData<bool>(UserID, "isBotDev")))
                {
                    if (UsersData.IsContainsKey($"LU_{cooldownParamName}", UserID))
                    {
                        ...
                        if (timeAfterUse.TotalSeconds >= userSecondsCooldown)
                        {
                            ...
                            {
                                FileTools.CreateFile(Bot.ChannelsPath + RoomID + "/CDD.json");
                                Dictionary<string, DateTime> list = new();
                                FileTools.SaveFile(Bot.ChannelsPath + RoomID + "/CDD.json", JsonConvert.SerializeObject(list));
                            }
                            ...
                            else
                            {
                                ...
                                {
                                    ConsoleServer.SendConsoleMessage("info", $"Пользователь {GetUsername(UserID, UserID)} попытался использовать команду, но она на глобальном кулдауне!");
                                    LogWorker.LogWarning($"Пользователь {GetUsername(UserID, UserID)} попытался использовать команду, но она на глобальном кулдауне!", cooldownParamName);
                                    return false;
                                }
                            }
                        }
                        else
                        {
                            ...
                            ConsoleServer.SendConsoleMessage("info", $"Пользователь {GetUsername(UserID, UserID)} попытался использовать команду, но она на кулдауне!");
                            LogWorker.LogWarning($"Пользователь {Tools.GetUsername(UserID, UserID)} попытался использовать команду, но она на кулдауне!", cooldownParamName);
                            return false;
                        }
                    }
                    else
                    ...
                    if (data.args.Count >= 1)
                    {
                        ...
                        while (ip_attempt < ips.Length && !success)
                        {
                            ...
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                var result = JsonConvert.DeserializeObject<GPT>(json);
            ...
            public static void MessageWorker(string UserID, string RoomId, string Username, string Message, OnMessageReceivedArgs e, string Room, string platform, string ServerChannel = "")
            {
                ...
                if (check)
                {
                    ...
                    FileTools.CreateDirectory(Bot.ChannelsPath + RoomId);
                    FileTools.CreateDirectory(Bot.ChannelsPath + RoomId + "/MSGS/");
                    FileTools.CreateDirectory(messagesSendedPath);

                    var OutPutMessage = "";

                    if (File.Exists(messagesSendedUserPath))
                    {
                        messagesSended = ToNumber(File.ReadAllText(messagesSendedUserPath)) + 1;
                    }
                    ...
                    else
                    {
                        ...
                        SaveBalance(UserID, 0, roundedNumber);
                        int floatBalance = GetFloatBalance(UserID);
                    ...
                    CAFUS.Maintrance(UserID, Username);
                    ...
                    foreach (Match match in matches)
                    {
                        ...
                        if (user.ToLower() != Username.ToLower() && File.Exists(filePath))
                        {
                            ...
                            SaveBalance(UserID, 0, 5);
                            SaveBalance(userID, 0, 8);
                    ...
                    {
                        ErrorOccured(ex.Message, "boteventMSG0A");
                    }
                    if (platform == "tw")
                    {
                    ...
                    {
                        FileTools.SaveFile(N2IPath, UserID);
                        FileTools.SaveFile(I2NPath, Username);
                    }
                    UsersData.UserSaveData(UserID, "lastSeenChannel", Room, false);
                    FileTools.SaveFile(messagesSendedUserPath, messagesSended.ToString());
            ...
            public static List<(int Index, string Key, LocationCacheData Data)> Search(string query, Dictionary<string, LocationCacheData> dataDictionary)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Tools.ErrorOccured(ex.Message, "searchLoc");
                    return result;
                }
            }
        ...
        public class TwitchTokenGetter
        {
