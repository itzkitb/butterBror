    ...
    public partial class Commands
    {
        public class BotCommand
        {
            ...
            public static CommandReturn Index(CommandData data)
            {
                ...
                try
                {
                    ...
                    if (args.Count > 0)
                    {
                        ...
                        else if (argument1.Contains("verify"))
                        {
                            ...
                            FileTools.CreateDirectory(Bot.ProgramPath + $"TwitchVerifyWaiting/");
                        ...
                        else if (currencyAlias.Contains(argument1))
                        {
                            if (argsc > 2)
                            {
                                ...
                                if (addDollarsAlias.Contains(argument2))
                                {
                                    if (is_dev)
                                    {
                                        var converted = Tools.ToNumber(args.ElementAt(2).ToLower());
                        ...
                        else if (is_mod)
                        {
                            if (argument1 == "ban")
                            {
                                if (args.Count > 1)
                                {
                                    ...
                                    var BanChannelID = Tools.GetUserID(argument2);
                            ...
                            else if (argument1 == "rejoinchannel" && data.Platform == Platforms.Twitch)
                            {
                                try
                                {
                                    ...
                                    if (Tools.FilterTextWithoutSpaces(user) != "")
                                    {
                                ...
                                {
                                    Tools.ErrorOccured(ex.Message, "BOTCMD/rejoin");
                                }
                            }
                            else if (argument1 == "addchannel" && data.Platform == Platforms.Twitch)
                            {
                                try
                                {
                                    ...
                                    if (Tools.FilterTextWithoutSpaces(user) != "")
                                    {
                                        var userID = Tools.GetUserID(user);
                                        if (userID != "err")
                                        {
                                            ...
                                            Bot.client.JoinChannel(user);
                                            Tools.SendMsgReply(Channel, RoomId, TranslationManager.GetTranslation(data.User.Lang, "addedChannel", data.ChannelID).Replace("%user%", user), MsgId, data.User.Lang, true);
                                            Thread.Sleep(1000);
                                            Tools.ChangeNicknameColorAsync(ChatColorPresets.DodgerBlue);
                                            Tools.SendMessage(user, TranslationManager.GetTranslation(data.User.Lang, "welcomeChannel", data.ChannelID).Replace("%version%", BotEngine.botVersion), RoomId, MsgId, data.User.Lang, true);
                                        }
                                        else
                                        {
                                            Tools.SendMsgReply(Channel, RoomId, TranslationManager.GetTranslation(data.User.Lang, "noneExistUser", data.ChannelID).Replace("%user%", user), MsgId, data.User.Lang, true);
                                        }
                                    }
                                }
                                catch (Exception ex)
                                {
                                    Tools.ErrorOccured(ex.Message, "BOTCMD/join");
                                }
                            }
                                ...
                                try
                                {
                                    ...
                                    if (Tools.FilterTextWithoutSpaces(user) != "")
                                    {
                                        ...
                                        if (userID != "err")
                                        {
                                            ...
                                            DataManager.SaveData(Bot.SettingsPath, "channels", output);
                                            Bot.client.LeaveChannel(user);
                                            Tools.SendMsgReply(Channel, RoomId, TranslationManager.GetTranslation(data.User.Lang, "delChannel", data.ChannelID).Replace("%user%", user), MsgId, data.User.Lang, true);
                                        }
                                        else
                                        {
                                            Tools.SendMsgReply(Channel, RoomId, TranslationManager.GetTranslation(data.User.Lang, "noneExistUser", data.ChannelID).Replace("%user%", user), MsgId, data.User.Lang, true);
                            ...
                            else if (argument1 == "joinchannel" && data.Platform == Platforms.Twitch)
                            {
                                try
                                {
                                    ...
                                    {
                                        Bot.client.JoinChannel(user);
                                        Tools.SendMsgReply(Channel, RoomId, TranslationManager.GetTranslation(data.User.Lang, "joinedChannel", data.ChannelID), MsgId, data.User.Lang, true);
                                    }
                                }
                                catch (Exception ex)
                                {
                            ...
                            else if (argument1 == "leavechannel" && data.Platform == Platforms.Twitch)
                            {
                                try
                                {
                                    ...
                                    {
                                        Bot.client.LeaveChannel(user);
                                        Tools.SendMsgReply(Channel, RoomId, TranslationManager.GetTranslation(data.User.Lang, "leavedChannel", data.ChannelID), MsgId, data.User.Lang, true);
                                    }
                                }
                                catch (Exception ex)
                                {
                                    Tools.ErrorOccured(ex.Message, "BOTCMD/leave");
                                }
                            }
                            else if (is_dev)
                            {
                                if (argument1 == "modadd")
                                {
                                    ...
                                    {
                                        Tools.ErrorOccured(ex.Message, "BOTCMD/modadd");
                                    }
                                }
                                else if (argument1 == "demod")
                                {
                                    ...
                                    {
                                        Tools.ErrorOccured(ex.Message, "BOTCMD/moddel");
                                    }
                                }
                ...
                {
                    LogWorker.LogError(ex.Message, "cmd6A");
