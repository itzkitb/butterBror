    ...
    public partial class Commands
    {
        public class FrogGame
        {
            public static CommandInfo Info = new()
            {
                ...
                UseURL = "https://itzkitb.ru/bot_command/frogs",
                UserCooldown = 10,
                GlobalCooldown = 1,
            ...
            public static CommandReturn Index(CommandData data)
            {
                ...
                {
                    frogsBalance = long.Parse(frogsBalanceCache.Value);
                    frogsGifted = long.Parse(frogsGiftedCache.Value);
                    frogsReceived = long.Parse(frogsReceivedCache.Value);
                }
                    ...
                    else if (giftAliases.Contains(data.args[0].ToLower()))
                    {
                        if (data.args.Count > 2)
                        {
                            ...
                            else if (frogsBalance >= frogs)
                            {
                                ...
                                dynamic? giftUserFrogsBalanceCache = jsonManager.GetData<dynamic>("frogs");
                                dynamic? giftUserFrogsReceivedCache = jsonManager.GetData<dynamic>("received");
                                ...
                                {
                                    giftUserFrogsBalance = long.Parse(giftUserFrogsBalanceCache.Value);
                                    giftUserReceived = long.Parse(giftUserFrogsReceivedCache.Value);
                                }
