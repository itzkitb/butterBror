...
namespace butterBror
{
    ...
    {
        static async void fishing(OnChatCommandReceivedArgs e, string lang)
        {
            string[] moveAlias = ["move", "двигаться", "place", "место", "m", "д", "p", "м"];
            if (await CommandUtil.IsNotOnCooldown(10, 1, "Fishing", e.Command.ChatMessage.UserId, e.Command.ChatMessage.RoomId, Platforms.Twitch))
            {
