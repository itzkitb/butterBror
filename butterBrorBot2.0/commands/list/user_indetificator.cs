            ...
            public CommandReturn Index(CommandData data)
            {
                ...
                try
                {
                    if (data.arguments.Count > 0)
                    {
                        ...
                        {
                            commandReturn.SetMessage(TranslationManager.GetTranslation(data.user.language, "error:user_not_found", data.channel_id, data.platform).Replace("%user%", username));
                            commandReturn.SetColor(ChatColorPresets.CadetBlue);
                        }
                        else
                        {
                            commandReturn.SetMessage(TranslationManager.GetTranslation(data.user.language, "command:id:user", data.channel_id, data.platform).Replace("%id%", ID).Replace("%user%", Names.DontPing(username)));
                        }
                    }
                    else
                    {
